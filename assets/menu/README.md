...
- `fonts/`: self-hosted IBM Plex Mono (regular, medium, semibold), from the
...
- `rat.webp`: the previous static sidebar mascot generated with the built-in ImageGen tool from
  ...
  without text, borders, or other UI.
- `goose-mayhem.webp`: a screenshot of the existing Goose Mayhem game.
- `research-placeholder.svg`, `vision-placeholder.svg`: schematic placeholder
  illustrations, not actual research results.
- `monogram.svg`: the reference's pixel-style JY mark recreated as SVG.

The original game banners remain in `assets/button-images/`.
